...
def _compute_attention(attention_mechanism, cell_output, previous_alignments,
  ...
  """Computes the attention and alignments for a given attention_mechanism for Soft Attention."""
  ...
  #Apply output_layer to compute logits
  logits=output_layer(attention)

  return attention, alignments, logits
...
def _compute_beam_joint_attention(attention_mechanism, cell_output, previous_alignments,
  ...
  if attention_layer is not None:
    ...
    if output_layer is not None:
      ...
      logits = tf.verify_tensor_all_finite( logits, msg="LogLogits failed")
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def __init__(self,
    ...
    **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
    ...
    self._attention_architecture=attention_architecture
  ...
  def call(self, inputs, state):
    ...
    for i, attention_mechanism in enumerate(self._attention_mechanisms):
      if self._attention_architecture == "joint":
        #Call _compute_beam_joint_attention for Beam-Joint Attention
        attention, alignments,logits  = _compute_beam_joint_attention(
            attention_mechanism, cell_output, previous_alignments[i],
            self._attention_layers[i] if self._attention_layers else None, output_layer=self._output_layer)
      elif self._attention_architecture == "standard":
        #Call _compute_attention for Soft Attention
        attention, alignments,logits  = _compute_attention(
            attention_mechanism, cell_output, previous_alignments[i],
            self._attention_layers[i] if self._attention_layers else None, output_layer=self._output_layer)
