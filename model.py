...
class BaseModel(object):
    ...
    with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        with tf.device(model_helper.get_device_str(num_layers - 1, num_gpus)):
          loss = 0 
          if hparams.attention_architecture == "joint" and self.mode == tf.contrib.learn.ModeKeys.TRAIN:
          ...
          elif hparams.attention_architecture == "joint" and self.mode != tf.contrib.learn.ModeKeys.TRAIN:
  ...
  def _build_encoder_cell(self, hparams, num_layers, num_residual_layers,
    ...
    return model_helper.create_rnn_cell(
        ...
        num_units=hparams.num_units,   ###Changed for ICLR 2018 Phrase Translation
  ...
  def _build_decoder(self, encoder_outputs, encoder_state, hparams):
    ...
    with tf.variable_scope("decoder") as decoder_scope:
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        if self.mode != tf.contrib.learn.ModeKeys.INFER:
            decoder_emb_inp = tf.concat(values=[decoder_emb_inp, tf.expand_dims(tf.cast(target_input,tf.float32),2)], axis=-1)
        ...
        with tf.device(model_helper.get_device_str(device_id, num_gpus)):
            logits = outputs.rnn_output #self.output_layer(outputs.rnn_output)
      ...
      ## Inference
        ...
        if beam_width > 0:
          ...
          my_decoder = tf.contrib.seq2seq.BasicDecoder(
              ...
              helper,
              decoder_initial_state,
              output_layer=None, # applied per timestep
          )
...
class Model(BaseModel):
  ...
  def _build_encoder(self, hparams, stop=False):
    """Build an encoder."""
    num_layers = hparams.num_layers
    ...
    with tf.variable_scope("encoder") as scope:
      ...
      if hparams.attention_architecture == "joint" and stop:
